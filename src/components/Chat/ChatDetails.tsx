    ...
    >
      {isInfoVisible && (
        <Block className="chat-info px-4 py-5 min-h-[54%]">
          <header>
            ...
            <Delete className="text-[22px] right-4 top-3" handler={toggleSetInfoVisible} />
          </header>
          <main className="chat-info__content pb-8 mt-3 h-full overflow-auto">
            ...
            <div className="details flex flex-col mt-4 gap-2">
              {DETAILS.map((detail) => {
                ...
                return <ChatDetailsItem key={detail.name} detail={detail} value={value} />;
              })}
            </div>
          </main>
        </Block>
      ...
      {isMembersVisible && (
        <Block className="chat-members px-4 py-5">
          ...
          <Delete className="text-[22px] right-4 top-3" handler={toggleSetMembersVisible} />
        </Block>
      )}
    </section>
  );
